...
const MyOrder = (props: any) => {
    ...
    const getCustomrOrderAPI = async () => {
        ...
        try {
            ...
            console.log('ordeeerrrDataJSON->', DataJSON?.orders);
            setorderData(DataJSON?.orders);
            if (response.status === 200) {
                setLoader(false);
                setorderData(DataJSON?.orders);
                const consultationOrders = DataJSON?.orders?.filter((order: any) => order.order_type === "consultation");
